...
class NoxApp(ui.View):
    ...
    def _resolve_pending(self):
        """
        Только координация: сложить ссылки в очередь разбора и вернуть
        управление. Ни DNS, ни SSL, ни yt-dlp здесь больше нет.

        Раньше extract_info выполнялся прямо отсюда, синхронно. Один
        handshake VK на 60 секунд замораживал весь интерфейс, хотя
        соседняя загрузка всё это время спокойно качалась своим потоком.
        Теперь разбором занят отдельный последовательный поток, а такт
        только ставит ему задачу.
        """
        if self._resolving:
            return
        if DOWNLOADER.signature() != self._last_sig:
